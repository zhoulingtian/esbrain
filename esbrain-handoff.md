...
### 2026-08-03 更新：第八轮学习路径与辅助

- 首次启动最后一步收集目标级别、每日 15/30/60 分钟与学习偏好；首页计划用真实到期词数调节复习量，并始终保留跳过和自由选课。
- 错题本按词汇、语法专项、听力分组；`skillMistakeLog` 记录真实专项错题事件，周报展示专项/听力错题、复习自评准确率和基于阈值的下周建议。
- 设置页新增 GitHub 私密 Gist 同步。token 只保存在本机学习状态，上传会剥离 token；`lastSync` 分叉时显示两边的复习、打卡和最近学习统计，必须选择本地上传、采用云端或导出手动合并。
- 最新验证：`node build/assemble.js` 全绿，`node build/smoke.js` 为 `87 passed, 0 failed`；缓存版本为 `v12`。
